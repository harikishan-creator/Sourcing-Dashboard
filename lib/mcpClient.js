...
let _initialized = false;  // only call initialize once per lambda
...
async function mcpCall(method, params = {}) {
  ...
  if (json.error) {
    ...
    // Treat "already initialized" as success — session is active
    if (msg.includes('already initialized')) return json.result ?? {};
    throw new Error(msg);
  }
  return json.result;
}
...
// ── Initialize session ───────────────────────────────────────────────────────
// This MCP server uses stateless Bearer token auth — no initialize needed.
export async function init() {
  // No-op: corona-staging MCP server does not require initialize handshake
  // when using Bearer token auth. Calling it causes "already initialized" errors.
  return;
}
