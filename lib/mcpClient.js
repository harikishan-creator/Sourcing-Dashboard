...
export async function fetchInventory() {
  ...
  const _rawText = extractText(jobRes);
  const jobCode = extractJobCode(_rawText);
  if (!jobCode) throw new Error('No jobCode. Raw response: ' + JSON.stringify(jobRes).slice(0, 500) + ' | text: ' + _rawText.slice(0, 300));
