...
// Raw MCP call passthrough (for discovery only)
export async function mcpRaw(method, params = {}) {
  await init();
...
export async function triggerJob(type, facility) {
  ...
  } else if (type === 'drr7' || type === 'drr48h') {
    // drr48h = last 48 hours only (fast refresh - fewer rows)
    const startDate = type === 'drr48h' ? new Date(now.getTime() - 48*3600*1000) : d7;
